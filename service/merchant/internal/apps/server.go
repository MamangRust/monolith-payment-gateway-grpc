...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	pb.RegisterMerchantQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterMerchantCommandServiceServer(grpcServer, s.Handlers)
	pbdocument.RegisterMerchantDocumentQueryServiceServer(grpcServer, s.Handlers)
	pbdocument.RegisterMerchantDocumentCommandServiceServer(grpcServer, s.Handlers)
	pb.RegisterMerchantTransactionServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterMerchantStatsAmountServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterMerchantStatsMethodServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterMerchantStatsTotalAmountServiceServer(grpcServer, s.Handlers)
...
func InitConfig() (*Config, error) {
	...
	v.SetDefault("service_name", "merchant-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_MERCHANT"), viper.GetString("REDIS_PORT_MERCHANT")),
		Password:     viper.GetString("REDIS_PASSWORD_MERCHANT"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_MERCHANT"), viper.GetString("REDIS_PORT_MERCHANT"))),
		zap.Int("db", viper.GetInt("REDIS_DB_MERCHANT")),
	)
