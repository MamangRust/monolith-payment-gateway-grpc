...
func NewServer(cfg *Config) (*Server, error) {
	...
	services := service.NewService(&service.Deps{Cache: cacheStore, Hash: hasher, Repositories: repositories, Logger: logger})
...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	pb.RegisterUserQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterUserCommandServiceServer(grpcServer, s.Handlers)
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_USER"), viper.GetString("REDIS_PORT_USER"))),
		zap.Int("db", viper.GetInt("REDIS_DB_USER")),
	)
