...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	pb.RegisterTopupQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterTopupCommandServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTopupStatsAmountServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTopupStatsMethodServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTopupStatsStatusServiceServer(grpcServer, s.Handlers)
...
func InitConfig() (*Config, error) {
	...
	v.SetDefault("service_name", "topup-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_TOPUP"), viper.GetString("REDIS_PORT_TOPUP")),
		Password:     viper.GetString("REDIS_PASSWORD_TOPUP"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_TOPUP"), viper.GetString("REDIS_PORT_TOPUP"))),
		zap.Int("db", viper.GetInt("REDIS_DB_TOPUP")),
	)
