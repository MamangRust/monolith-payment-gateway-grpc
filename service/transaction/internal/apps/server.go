...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	...
	pb.RegisterTransactionQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterTransactionCommandServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTransactionStatsAmountServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTransactionStatsMethodServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterTransactionStatsStatusServiceServer(grpcServer, s.Handlers)
...
func InitConfig() (*Config, error) {
	...
	v.SetDefault("service_name", "transaction-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_TRANSACTION"), viper.GetString("REDIS_PORT_TRANSACTION")),
		Password:     viper.GetString("REDIS_PASSWORD_TRANSACTION"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_TRANSACTION"), viper.GetString("REDIS_PORT_TRANSACTION"))),
		zap.Int("db", viper.GetInt("REDIS_DB_TRANSACTION")),
	)
