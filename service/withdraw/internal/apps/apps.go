...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	pb.RegisterWithdrawQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterWithdrawCommandServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterWithdrawStatsAmountServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterWithdrawStatsStatusServiceServer(grpcServer, s.Handlers)
...
func InitConfig() (*Config, error) {
	...
	v.SetDefault("service_name", "withdraw-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_WITHDRAW"), viper.GetString("REDIS_PORT_WITHDRAW")),
		Password:     viper.GetString("REDIS_PASSWORD_WITHDRAW"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_WITHDRAW"), viper.GetString("REDIS_PORT_WITHDRAW"))),
		zap.Int("db", viper.GetInt("REDIS_DB_WITHDRAW")),
	)
