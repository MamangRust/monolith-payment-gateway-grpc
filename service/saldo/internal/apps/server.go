...
func (s *Server) registerServices(grpcServer *grpc.Server) {
	pb.RegisterSaldoQueryServiceServer(grpcServer, s.Handlers)
	pb.RegisterSaldoCommandServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterSaldoStatsBalanceServiceServer(grpcServer, s.Handlers)
	pbstats.RegisterSaldoStatsTotalBalanceServer(grpcServer, s.Handlers)
...
func InitConfig() (*Config, error) {
	...
	v.SetDefault("service_name", "saldo-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_SALDO"), viper.GetString("REDIS_PORT_SALDO")),
		Password:     viper.GetString("REDIS_PASSWORD_SALDO"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_SALDO"), viper.GetString("REDIS_PORT_SALDO"))),
		zap.Int("db", viper.GetInt("REDIS_DB_SALDO")),
	)
