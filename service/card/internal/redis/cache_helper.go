...
type CacheStore struct {
	ctx      context.Context
	mencache *redis.Client
	logger   logger.LoggerInterface
}

func NewCacheStore(ctx context.Context, mencache *redis.Client, logger logger.LoggerInterface) *CacheStore {
	return &CacheStore{ctx: ctx, mencache: mencache, logger: logger}
}

func GetFromCache[T any](store *CacheStore, key string) (*T, bool) {
	cached, err := store.mencache.Get(store.ctx, key).Result()
	if err == redis.Nil {
		return nil, false
	}
	if err != nil {
...
func SetToCache[T any](store *CacheStore, key string, data *T, expiration time.Duration) {
	...
	if err := store.mencache.Set(store.ctx, key, jsonData, expiration).Err(); err != nil {
...
func DeleteFromCache(store *CacheStore, key string) {
	if err := store.mencache.Del(store.ctx, key).Err(); err != nil {
