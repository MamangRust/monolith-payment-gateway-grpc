...
func (s *Server) Run() error {
	...
	s.registerServices(grpcServer, s.Handlers)
...
func (s *Server) registerServices(grpc *grpc.Server, handler handler.Handler) {
...
func InitConfig(service string) (*Config, error) {
	v := viper.New()

	v.SetDefault("service_name", "card-service")
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	client := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_CARD"), viper.GetString("REDIS_PORT_CARD")),
		Password:     viper.GetString("REDIS_PASSWORD_CARD"),
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_CARD"), viper.GetString("REDIS_PORT_CARD"))),
		zap.Int("db", viper.GetInt("REDIS_DB_CARD")),
	)
